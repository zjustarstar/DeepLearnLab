...
class generator(nn.Module):
    def __init__(self, input_dim=100, out_dim=1,
        ...
        self.deconv = nn.Sequential(
            # 1*1->4*4
            nn.ConvTranspose2d(input_dim+class_num, 256, 4, 1, 0),
            nn.BatchNorm2d(256),
            nn.ReLU(),
            # 4*4->8*8
    ...
    for e in range(epoch):
        ...
        for step, (features, labels) in enumerate(train_iter):
            ...
            y_fill_ = y_vec_.unsqueeze(2).unsqueeze(3).\
                expand(batch_size, class_num, out_img_size, out_img_size)

            # ===================判别器网络训练====================
            optimizer_D.zero_grad()

            D_real_out = D(features, y_fill_)
            D_real_out = D_real_out.squeeze(2).squeeze(2)
            ...
            D_z_out = D_z_out.squeeze(2).squeeze(2)
            ...
            D_fake_out = D_fake_out.squeeze(2).squeeze(2)
